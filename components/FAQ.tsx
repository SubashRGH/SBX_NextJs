...
const LIST = [
  {
    faq_q: "Lorem ipsum dolor sit amet, consetetur?",
    faq_a:
...
export default function FAQ() {
  ...
  const faq = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/SiteAPI/SearchFAQ`, {
      ...
      .then((res) => res.json())
      .then((response) => {
        if (router.pathname === "/accounts/account-type") {
          const general = response?.data?.filter(
            (res: any) => res.faq_type === "Accounts"
          );
          setList(general);
        } else if (router.pathname === "/partner") {
          const general = response?.data?.filter(
            (res: any) => res.faq_type === "INTRODUCING_BROKERS"
          );
          setList(general);
        } else if (router.pathname === "/platform/metatrader4") {
          const general = response?.data?.filter(
            (res: any) => res.faq_type === "MT4"
          );
          setList(general);
        } else if (router.pathname === "/platform/metatrader5") {
          const general = response?.data?.filter(
            (res: any) => res.faq_type === "MT5"
          );
          setList(general);
        } else if (router.pathname === "/platform/c-trader") {
          const general = response?.data?.filter(
            (res: any) => res.faq_type === "cTrader"
          );
          setList(general);
        }
      });
